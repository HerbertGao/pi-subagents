...
### Fixed
- **The `Agent` tool no longer over-recommends worktree isolation in directories that cannot support it.** Its full and compact descriptions, parameter schema, and generated-agent template now reserve `isolation: "worktree"` for file-modifying tasks in Git repositories that need an isolated copy, and explicitly tell the orchestrator to omit it for read-only tasks and non-Git directories. Explicit worktree requests remain strict and still fail clearly when Git cannot create the requested isolation.
